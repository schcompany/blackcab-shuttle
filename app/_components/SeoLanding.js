...
export default function SeoLanding({ title, intro, sections, url, serviceType, image = "/images/1.jpg" }) {
  ...
  return (
    <main className="min-h-screen bg-white pb-24 text-[#23231f] md:pb-0">
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schema) }} />
      <header className="sticky top-0 z-40 border-b border-[#e6e0d4] bg-white/95 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-5 py-4">
          <Link href="/" className="leading-none">
            <div className="text-xl font-black">BLACKCAB</div>
            <div className="text-[9px] font-bold uppercase tracking-[0.3em] text-[#806020]">Shuttle Brussels</div>
          </Link>
          <div className="flex gap-2">
            <a href={`tel:${PHONE}`} className="rounded-full bg-white px-4 py-2 text-sm font-black text-black">Appeler</a>
            <Link href="/#reservation" className="hidden rounded-full bg-[#806020] px-4 py-2 text-sm font-black text-white sm:inline-flex">Réserver</Link>
          </div>
        </div>
      </header>

      <article className="mx-auto max-w-6xl px-5 py-8 md:py-14">
        <div className="grid items-center gap-8 overflow-hidden rounded-[28px] bg-[#151513] p-6 text-white lg:grid-cols-[1.05fr_0.95fr] lg:p-10">
          <div>
            <p className="text-sm font-bold uppercase tracking-[0.2em] text-[#e6c987]">Service 24h/24 · Prix annoncé avant confirmation</p>
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              <Link href="/#reservation" className="rounded-full bg-[#806020] px-6 py-3 font-black text-white">Demander un devis</Link>
        ...
        <div className="mt-12 grid gap-5 md:grid-cols-2">
          {sections.map((section) => (
            <section key={section.heading} className="rounded-[20px] border border-[#e6e0d4] bg-[#f7f5ef] p-6">
        ...
        <nav aria-label="Services associés" className="mt-10 flex flex-wrap gap-3 border-t border-[#e6e0d4] pt-7 text-sm font-bold text-[#806020]">
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 grid grid-cols-2 gap-2 border-t border-[#e6e0d4] bg-white/95 p-2 md:hidden">
        <Link href="/#reservation" className="rounded-xl bg-[#806020] px-3 py-4 text-center text-sm font-black">Réserver</Link>
